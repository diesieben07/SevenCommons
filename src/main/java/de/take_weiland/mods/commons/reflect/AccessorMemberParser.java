...
final class AccessorMemberParser {
    ...
    private static void validate(Method method, MethodHandle target) {
        ...
        if (method.getReturnType() != targetType.returnType() && !method.getReturnType().isAssignableFrom(targetType.returnType())) {
        ...
        for (int i = 0; i < myParamCount; i++) {
            if (!targetType.parameterType(i).isAssignableFrom(myParams[i])) {
