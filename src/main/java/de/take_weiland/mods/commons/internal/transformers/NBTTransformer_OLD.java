 ...
 * @author diesieben07
 */
final class NBTTransformer_OLD {
	...
	private static final ClassInfo enumClassInfo = ClassInfo.of(Enum.class);
	private static final ClassInfo tileEntity = ClassInfo.of("net/minecraft/tileentity/TileEntity");
	private static final ClassInfo entity = ClassInfo.of("net/minecraft/entity/Entity");
	private static final ClassInfo entityProps = ClassInfo.of(entityPropsName);
	...
	static void transform(ClassNode clazz, ClassInfo classInfo) {
		...
		for (FieldNode field : clazz.fields) {
			...
			if (!ASMUtils.isPrimitive(actualType) && enumClassInfo.isAssignableFrom(ClassInfo.of(actualType.getInternalName()))) {
	...
	private static boolean isEntityPropsDirect(ClassNode clazz) {
		...
		for (String iface : clazz.interfaces) {
			if (checkHasProps(ClassInfo.of(iface))) {
				return true;
			}
		}
		return false;
	...
	private static boolean checkHasProps(ClassInfo clazz) {
		for (String iface : clazz.interfaces()) {
			if (iface.equals(entityPropsName) || checkHasProps(ClassInfo.of(iface))) {
				return true;
			}
		}
		return false;
