...
final class DefaultCompanionFactory implements CompanionFactory {
    ...
    private CompanionClassInfo getInfo(Class<?> clazz) {
        ...
        if (!companionClasses.containsKey(clazz)) {
            Map<Property<?>, TypeSyncer<?, ?, ?>> members = SyncCompanions.getSyncedMemberInfo(clazz);
            if (members.isEmpty()) {
                info = null;
            } else {
                // TODO!
                Class<?> companionClass = Object.class; //new BytecodeEmittingCompanionGenerator_OLD(this, clazz, members).generateCompanion();
    ...
    int getNextFreeIDFor(Class<?> clazz) {
        if (clazz == Object.class) {
            return SyncCompanion.FIRST_USABLE_ID; // ID 0 is taken for end of stream
        }
