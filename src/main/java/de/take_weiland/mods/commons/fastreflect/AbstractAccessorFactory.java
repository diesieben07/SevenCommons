...
abstract class AbstractAccessorFactory implements AccessorFactory {
	...
	InterfaceInfo analyze(Class<?> iface) {
		...
		for (Method method : iface.getMethods()) {
			...
			} else if (method.isAnnotationPresent(InvokeMethod.class)) {
	...
	Field findGetterTarget(Class<?> iface, Method getter) {
		...
		Field f = ReflectionHelper.findField(params[0], ann.names());
	...
	Method findInvokerTarget(Class<?> iface, Method invoker) {
		...
		InvokeMethod ann = invoker.getAnnotation(InvokeMethod.class);
		Method m = findMethod(params[0], ann.names(), Arrays.copyOfRange(params, 1, params.length));
