...
abstract class AbstractStrategy implements FastreflectStrategy {
	...
	Field findGetterTarget(Class<?> iface, Method getter) {
		...
		Field f = ReflectionHelper.findField(params[0], ann.srg() ? ASMNames.field(ann.field()) : ann.field());
	...
	Method findInvokerTarget(Class<?> iface, Method invoker) {
		...
		Method m = findMethod(params[0], ann.srg() ? ASMNames.method(ann.method()) : ann.method(), Arrays.copyOfRange(params, 1, params.length));
